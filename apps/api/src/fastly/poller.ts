// Periodic poller: keeps the latest Fastly snapshot + connector status in memory. Read-only; a
// poll failure retains the last good snapshot and is surfaced via status (never fabricated).
import type { FastlyClient, FastlySnapshot } from './types.js';

export interface FastlyConnectorStatus {
  enabled: boolean;
  running: boolean;
  source: FastlySnapshot['source'] | null;
  intervalMs: number;
...
export interface FastlyPollerDeps {
  client: FastlyClient;
  enabled: boolean;
  intervalMs: number;
...
export class FastlyPoller {
  private client: FastlyClient;
  private enabled: boolean;
  private intervalMs: number;
  private readonly now: () => number;
  private timer: ReturnType<typeof setInterval> | null = null;
  private latest: FastlySnapshot | null = null;
  ...
  constructor(private readonly deps: FastlyPollerDeps) {
  ...
  latestSnapshot(): FastlySnapshot | null {
    return this.latest;
  }
  ...
  async runOnce(correlationId?: string): Promise<{ ok: boolean; error?: string }> {
    ...
    try {
      ...
      this.deps.logger?.warn({ consecutiveFailures: this.consecutiveFailures }, 'fastly: poll failed');
  ...
  reconfigure(deps: { client: FastlyClient; enabled: boolean; intervalMs: number }): void {
    this.stop();
    this.client = deps.client;
    this.enabled = deps.enabled;
    this.intervalMs = deps.intervalMs;
  ...
  status(): FastlyConnectorStatus {
    ...
    return {
      ...
      serviceCount: this.latest?.summary.serviceCount ?? 0,
    };
  }
}
