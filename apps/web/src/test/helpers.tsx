...
export const NETWORK_DEVICES_BODY = {
  ...
  items: [
    ...
    { id: 'JPE00000002', hostname: 'edge2.dub.rte.ie', modelName: 'DCS-7280SR3', softwareVersion: '4.31.2F', deviceType: 'router', datacentre: 'Parkwest', streaming: true, reachable: true, freshness: cvFresh, observedAt: '2026-07-15T12:00:00Z', source: 'mock' },
  ],
};
...
export const PNI_HISTORY_BODY = {
  ...
  series: [
    ...
    { deviceId: 'JPE00000001', interfaceName: 'Ethernet8/3/1', provider: 'Eir', linkType: 'PRIVATE_PEERING', datacentre: 'Citywest', points: [
      { at: '2026-07-15T12:00:00Z', inBps: 0, outBps: 0 },
    ] },
  ...
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (p.endsWith('/api/v1/me')) body = principal;
        ...
        live: { slices: [
          ...
          { label: 'Fastly', kind: 'commercial', platform: 'Fastly', bps: 1e9, links: 1 },
          { label: 'Akamai', kind: 'commercial', platform: 'Akamai', bps: 5e8, links: 1 },
        ...
        { guid: 'RTEONE', title: 'RTÉ One', callSign: 'RTEONE', isVirtual: false, daiKey: 'OzeyKEY', delivery: 'dai' },
        { guid: 'RTENewsChannel', title: 'RTÉ News', callSign: 'RTENewsNow', isVirtual: false, daiKey: null, delivery: 'direct' },
      ] };
      else if (p.endsWith('/stream-assurance/resolve-channel')) body = { resolved: {
        callSign: 'RTEONE', mediaPid: 'be90cf62', entryUrl: 'https://www.rte.ie/x.mpd?token1=t', finalManifestUrl: 'https://dai.google.com/linear/dash/pa/event/OzeyKEY/stream/sess/manifest.mpd', redirects: ['https://www.rte.ie/x.mpd?token1=t'], adTags: ['https://pubads.g.doubleclick.net/gampad/live/ads?x=1'],
        manifest: { manifestUrl: 'https://dai.google.com/linear/dash/pa/event/OzeyKEY/stream/sess/manifest.mpd', baseUrl: 'https://live.rte.ie/live/b/vc11/vc11.isml/dash/', presentation: 'dynamic', drm: { defaultKid: 'df163382-1ddd-fdd5-bec9-822c1ec0f052', systems: [] }, representations: [
      ...
      else if (p.includes('/stream-assurance/profiles/') && init?.method === 'DELETE') body = { id: p.split('/').pop(), deleted: true };
        ...
        observations: [
          { endpointId: 'fastly-edge', provider: 'fastly', role: 'reference', reachable: true, httpStatus: 200, kid: 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', lastModified: 'Sun, 26 Jul 2026 12:00:00 GMT', cdn: { cdn: 'fastly', edge: 'hit', parent: 'unknown', fetchedFromOrigin: false, originIdentity: null, servedBy: 'cache-lhr-1', age: null }, forwardedHost: 'live.rte.host', originHost: 'live.rte.host', init: { majorBrand: 'cmfc', compatibleBrands: ['cmfc', 'iso6'], tracks: [{ trackId: 1, handler: 'vide', timescale: 90000, width: 1920, height: 1080, codec: 'avc1' }], cenc: { isProtected: true, scheme: 'cenc', defaultKid: 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', perSampleIvSize: 8, hasConstantIv: false }, pssh: [{ systemId: 'edef8ba9-79d6-4ace-a3c8-27dcd51d21ed', systemName: 'Widevine', version: 1, kids: ['aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa'], dataSize: 128 }], warnings: [] }, media: { requested: { dash: true, hls: false, fragment: true }, dash: { presentation: 'dynamic', publishTime: '2026-07-27T21:00:00Z', defaultKid: 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', bandwidths: [800000, 2400000, 6000000] }, hls: false, fragment: { sequenceNumber: 100, baseMediaDecodeTime: 900000, sampleCount: 48, hasTimeline: true }, status: { dash: 200, hls: null, fragment: 200 } }, headers: { 'x-cache': 'HIT', 'x-served-by': 'cache-lhr-1', 'set-cookie': '[redacted]' } },
          { endpointId: 'akamai-edge', provider: 'akamai', role: 'candidate', reachable: true, httpStatus: 200, kid: 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', lastModified: 'Wed, 01 Jan 2026 00:00:00 GMT', cdn: { cdn: 'akamai', edge: 'miss', parent: 'miss', fetchedFromOrigin: true, originIdentity: null, servedBy: null, age: null }, forwardedHost: 'live.rte.ie', originHost: 'live.rte.host', headers: { 'x-cache': 'TCP_MISS from a1.akamai', 'x-cache-remote': 'TCP_MISS from a2.akamai' } },
          // Regression: an observation whose `media` is from an OLDER run shape (no `status`/`requested`,
          // dash null). This must NOT blank the page (guards + error boundary).
          { endpointId: 'realta-old', provider: 'realta', role: 'candidate', reachable: true, httpStatus: 200, kid: 'cccccccc-cccc-cccc-cccc-cccccccccccc', lastModified: null, cdn: { cdn: 'realta', edge: 'unknown', parent: 'unknown', fetchedFromOrigin: false, originIdentity: null, servedBy: null, age: null }, media: { dash: null, hls: false, fragment: null } as never },
        ],
